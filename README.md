...
| `{Title}Api` trait | One method per `operationId` for server implementations |
| `{Title}Client` trait | One method per `operationId` for transport-agnostic client implementations |
...
| [`openapi-trait-client`](openapi-trait-client/) | Transport-agnostic client proc-macro — not for direct use |
